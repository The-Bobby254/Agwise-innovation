# This script prepares .apsimx files for INTERCROP factorial simulations
...
adjust_soil_profile <- function(soil_df, modify = c("crop", "soil"), epsilon = 0.001) {
...
process_grid_element_experiment <- function(i,path.to.extdata,path.to.temdata,zone,level2=NA,expfile_name,clck,Crops, varietyids,rep, fix_crop_or_soil_parm) {
  ...
  for (i in min(length(Crops), length(varietyids[varietyids != ""]))){
    node_name <- paste0(Crops[i], "SowingRule")
    apsimx::edit_apsimx(expfile_name, 
                        src.dir = pathOUT,
                        wrt.dir = pathOUT,
                        root = c("pd", "Base_one"),
                        node = "Manager",
                        manager.child = node_name,
...
#' 
#' 
apsimIntercrop <- function(country,useCaseName,Crops, AOI = FALSE, season=1,zone,level2=NA,pathIn_zone=T,expfile_name,clck,varietyids,rep,fix_crop_or_soil_parm) {
  Crop <- Crops[1]
  ...
  results <- future_lapply(indices, function(i) {
    ...
    process_grid_element_experiment(i,path.to.extdata=path.to.extdata,path.to.temdata=path.to.temdata,
                                    zone=zone,level2=level2,expfile_name=expfile_name,clck=clck, Crops=Crops,
                                    varietyids=varietyids,rep=rep,fix_crop_or_soil_parm=fix_crop_or_soil_parm)
