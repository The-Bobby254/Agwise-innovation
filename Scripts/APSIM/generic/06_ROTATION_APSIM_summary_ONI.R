# Aggregated APSIM output and ONI classification of aggregated APSIM output for INTERCROPS
...
packages_required <- c("tidyverse", "ggridges","patchwork", "Rmisc", "terra", "cowplot", "foreach","doParallel","future","future.apply", "rlang", "tools")
...
get_ONI <- function(country, useCaseName, Crops, expfile_name, 
                    AOI=TRUE, season, Plot=TRUE,
  ...
  if (justplot == FALSE) {
    ...
    apsim <- apsim %>%
      ...
      mutate(
        ...
        sow_day = as.integer(word(SowDate, 1)),
        sow_month = match(word(SowDate, 2), month.abb),
