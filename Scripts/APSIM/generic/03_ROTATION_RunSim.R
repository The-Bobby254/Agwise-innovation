# This script runs APSIM simulations for ROTATIONS

packages_required <- c("arrow", "tidyverse", "furrr", "future", "future.apply",
                       "apsimx", "jsonlite", "tools")
...
runapsim <-function(i,path.to.zone,expfile_name,AOI=TRUE){
  ...
  tryCatch({
    ...
    write_parquet(harvest_data, paste0("HarvestReport_", experiment_name, "_",
                                       i, ".parquet"))
    gc()
  }, error = function(e){
...
apsim.exec <- function(country, useCaseName, Crops, AOI = TRUE,expfile_name,varietyids, zone, level2=NA){  
  # Main crop
  Crop <- Crops[1]
