# This script runs APSIM simulations for INTERCROPS
...
merge_INTERCROP_APSIM_output <- function(country, useCaseName, Crops, expfile_name, AOI = FALSE, season = NULL, varietyids, zone_folder = TRUE, level2_folder = FALSE) {
  Crop <- Crops[1]
  ...
  if (AOI) {
    ...
    write_parquet(
      ...
      sink = paste0(
        dir_path,
        clean_expfile_name, "_fieldData.parquet"
      )
    )
  }
}
